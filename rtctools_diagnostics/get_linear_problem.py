    ...
    def post(self):
        ...
        for problem_and_results in self.problem_and_results_list:
            ...
            (
                ...
                upper_constraint_dict,
                lower_constraint_dict,
            ) = get_variables_in_active_constr(
                results, nlp, casadi_equations, self.lam_tol
            )
        ...
        for problem in self.problem_list:
            ...
            for k, v in indices.items():
                if isinstance(v, int):
                    ...
                    for i in range(
                        0, v.stop - v.start, 1 if v.step is None else v.step
                    ):
                        var_names.append("{}__{}".format(k, i))
                else:
                    for i in range(
                        0, v[-1] - v[0]
                    ):  # , 1 if v.step is None else v.step):
                        var_names.append("{}__{}".format(k, i))
            ...
            for i, j, c in zip(A_coo.row, A_coo.col, A_coo.data):
                constraints[i].extend(
                    ["+" if c > 0 else "-", str(abs(c)), var_names[j]]
                )
            ...
            for i in range(len(constraints)):
                ...
                if np.isfinite(lb) and np.isfinite(ub) and lb == ub:
                    constraints[i] = "{} = {}".format(c_str, lb - b_i)
                elif np.isfinite(lb) and np.isfinite(ub):
                    constraints[i] = "{} <= {} <= {}".format(lb - b_i, c_str, ub - b_i)
                elif np.isfinite(lb):
                    constraints[i] = "{} >= {}".format(c_str, lb - b_i)
                elif np.isfinite(ub):
                    constraints[i] = "{} <= {}".format(c_str, ub - b_i)
                else:
                    raise Exception(lb, b, constraints[i])
            constraints_str = "  " + "\n  ".join(constraints)

            # Bounds
            bounds = []
            for v, lb, ub in zip(var_names, lbx, ubx):
                bounds.append("{} <= {} <= {}".format(lb, v, ub))
            bounds_str = "  " + "\n  ".join(bounds)

            with open(
                os.path.join(
                    self._output_folder, "model_for_priority_{}.lp".format(priority)
                ),
                "w",
            ) as f:
                f.write("Minimize\n")
                ...
                if "discrete" in casadi_equations.keys():
                    expand_discrete = casadi_equations[
                        "discrete"
